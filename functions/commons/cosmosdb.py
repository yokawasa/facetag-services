...
class AssetDB(AzureCosmosDB):
  ...
  def get_user_id(self, container_name):
    query = {'query': "SELECT * FROM s WHERE s.id='{}'".format(container_name)}
    try:
      doc = self.get_document(query)
      if doc:
        return doc['user_id']
    except Exception as e:
      logging.error("AssetDB get_user_id error ( asset_id {} ): {} ".format(container_name, e))
    return None

  def add_asset(self, asset_id, user_id):
    doc = {
      "id" : asset_id,
      "user_id": user_id
    }
